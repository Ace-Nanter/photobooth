...
* **Section supérieure en deux moitiés égales (50/50)** :
  * **Moitié gauche** : titre "Recevoir un email avec le lien de toutes les photos !" (`headlineSmall`) + champ email avec **icône Send intégrée en trailing** (même ligne). La touche IME du clavier Android est `Send` (déclenche la même action). Validation email basique (format). Si valide → **`AlertDialog` de confirmation** : "Le lien sera envoyé à \<email\>. Êtes-vous sûr ?" avec boutons "Oui"/"Non". Clic "Non" → ferme la dialog, l'email est conservé. Clic "Oui" → appel API, puis **Toast** "Email sauvegardé" en succès ou Toast avec message d'erreur en cas d'échec. L'envoi email est un appel API POST vers un webservice à définir ultérieurement (TODO).
