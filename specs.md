...
* **Section supérieure en deux moitiés égales (50/50)** :
  * **Moitié gauche** : titre "Recevoir un email avec le lien de toutes les photos !" (`headlineSmall`) + champ email avec **icône Send intégrée en trailing** (même ligne). La touche IME du clavier Android est `Send` (déclenche la même action). Validation email basique (format). Si valide → **`AlertDialog` de confirmation** : "Le lien sera envoyé à \<email\>. Êtes-vous sûr ?" avec boutons "Oui"/"Non". Clic "Non" → ferme la dialog, l'email est conservé. Clic "Oui" → appel `EmailService.sendEmailLink(email)`, puis **Toast** "Email sauvegardé" en succès ou Toast avec message d'erreur en cas d'échec HTTP.
  * **EmailService** (`data/remote/EmailService.kt`) : `POST EMAIL_WEBHOOK_URL` avec header `x-api-key: EMAIL_WEBHOOK_API_KEY` et body `{"email":"<adresse>"}`. Utilise `NetworkClient.photoClient`.
  * **Secrets** : URL et clé API stockées dans `secrets.properties` (gitignore) et injectées en `BuildConfig.EMAIL_WEBHOOK_URL` / `BuildConfig.EMAIL_WEBHOOK_API_KEY` via `build.gradle.kts`. Un fichier `secrets.properties.example` (sans vraies valeurs) est commité comme template.
